...
async def signup(req: SignupRequest):
    ...
    try:
        async with get_db_connection() as conn:
            ...
            # ✅ NOTE: firstname doesn't need to be unique!
            # Multiple users can have same firstname (Ali, Ahmed, etc.)
            # Only email must be unique for login purposes
            ...
            logger.info(f"✅ User signup successful: {user_id}, email: {req.email}")
...
async def login(req: LoginRequest):
    ...
    try:
        async with get_db_connection() as conn:
            ...
            # ✅ FIXED: Return lastName from database
            return AuthResponse(
                token=token,
                ...
                lastName=user['last_name']  # ✅ FIXED: Added this field
            )
...
async def google_signin(req: GoogleSignInRequest):
    ...
    try:
        ...
        async with get_db_connection() as conn:
            ...
            logger.info(f"✅ Google signin successful: {user['user_id']}")

            # ✅ FIXED: Added lastName (None for Google users)
            return AuthResponse(
                token=token,
                ...
                user_id=user['user_id'],
                firstname=user['firstname'],
                lastName=None  # ✅ FIXED: Added this field
            )
...
async def delete_bookmark(req: BookmarkDeleteRequest, user: dict = Depends(get_current_user)):
    """Delete a bookmark"""
    async with get_db_connection() as conn:
        # First get bookmark details before deleting
        ...
        # Create notification for deleted bookmark
        notif_id = generate_notification_id()
        surah_name_eng = bookmark['surah_name_eng']
        ayah_no = bookmark['ayah_no']
        
        await conn.execute("""
            INSERT INTO notifications (notification_id, user_id, title, message, created_at)
            VALUES ($1, $2, $3, $4, $5)
        """, notif_id, user['user_id'], "Bookmark Removed",
            f"You removed {surah_name_eng}, Ayah {ayah_no} from your bookmarks", 
            datetime.utcnow())
...
async def edit_profile(
    ...
    Handles both Image Upload and Data Update in one request (multipart/form-data)
    """
    try:
        async with get_db_connection() as conn:
            updates = []
            ...
            if dateofBirth:
                  dob_str = dateofBirth.strip()
                  try:
                    dob = parse(dob_str, dayfirst=False).date()
                  except Exception:
                    raise HTTPException(
                          status_code=400,
                          detail=f"Invalid date format: '{dateofBirth}'. Use YYYY-MM-DD"
                    )
...
async def upload_profile_image(
    ...
    try:
        ...
        async with get_db_connection() as conn:
            old_image_url = await conn.fetchval("SELECT profile_image_url FROM users WHERE user_id = $1", user['user_id'])
...
async def get_profile_image_redirect(user_id: str):
    ...
    try:
        async with get_db_connection() as conn:
            image_url = await conn.fetchval("SELECT profile_image_url FROM users WHERE user_id = $1", user_id)
...
async def delete_profile_image(user: dict = Depends(get_current_user)):
    ...
    try:
        async with get_db_connection() as conn:
            image_url = await conn.fetchval("SELECT profile_image_url FROM users WHERE user_id = $1", user['user_id'])
            ...
            await conn.execute("UPDATE users SET profile_image_url = NULL, updated_at = NOW() WHERE user_id = $1", user['user_id'])
            
            return SuccessResponse(message="Profile image deleted successfully", timestamp=datetime.utcnow())
