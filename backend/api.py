...
async def create_bookmark(req: BookmarkCreate, user: dict = Depends(get_current_user)):
    ...
    async with get_db_connection() as conn:
        ...
        if existing:
            raise HTTPException(status_code=400, detail="This verse is already bookmarked")
...
async def get_bookmark_by_id(bookmark_id: str, user: dict = Depends(get_current_user)):
...
@bookmark_router.delete("/{bookmark_id}", response_model=SuccessResponse)
async def delete_bookmark_by_id(bookmark_id: str, user: dict = Depends(get_current_user)):
    """Delete a bookmark by ID (path parameter)"""
    async with get_db_connection() as conn:
        result = await conn.execute("""
            DELETE FROM bookmarks 
            WHERE bookmark_id = $1 AND user_id = $2
...
@bookmark_router.delete("", response_model=SuccessResponse)
async def delete_bookmark(req: BookmarkDeleteRequest, user: dict = Depends(get_current_user)):
    """Delete a bookmark (body parameter - backward compatibility)"""
    async with get_db_connection() as conn:
        result = await conn.execute("""
            DELETE FROM bookmarks 
            WHERE bookmark_id = $1 AND user_id = $2
...
@profile_router.put("/profile", response_model=SuccessResponse)
async def update_profile(req: ProfileUpdate, user: dict = Depends(get_current_user)):
    """Update user profile - General update endpoint"""
    updates = []
    values = []
    ...
    async with get_db_connection() as conn:
        ...
        if req.bio is not None:  # Allow empty string
            updates.append(f"bio = ${len(values) + 1}")
            values.append(req.bio)
...
async def edit_profile(req: EditProfileRequest, user: dict = Depends(get_current_user)):
    """
    Edit Profile API - Restricted fields update (6 fields only):
    1. email
    2. image (profile_picture)
    3. username
    4. contact (phone_number)
