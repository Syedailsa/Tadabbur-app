...
#                         # 👇 CALL THE NEW AGENT HERE 👇
#                         # Note: We await it because it's an async LLM call
#                         clean_text = await clean_text_with_groq(raw_text)
                        
#                         logger.info(f"🎤 Stream audio for: {clean_text[:50]}...")
                        
#                         try:
#                             # Stream the CLEANED text to Murf
...
def extract_audio_data(response_text: str) -> Optional[dict]:
    ...
    Uses the robust SURAH_NAMES dictionary to map names to numbers.
    """
    ...
    if "🎧" in response_text or "http" in response_text:
        ...
        # 1. Regex to capture the Surah Name
        # Captures "Al-Baqarah", "Baqarah", "Yasin", etc.
        # Allows letters, hyphens, and apostrophes.
        surah_match = re.search(r'(?:Surah|surah)\s+([a-zA-Z\-\’\']+)', response_text)
        
        # 2. Regex to capture the Ayah Number
        ayah_match = re.search(r'(?:Ayah|ayah)\s*(\d+)', response_text)
        
        # 3. Check for URL
        url_pattern = r'https?://[^\s<>"{}|\\^`\[\]]+'
        urls = re.findall(url_pattern, response_text)

        if urls and surah_match:
            surah_name_raw = surah_match.group(1).strip()
            
            # --- LOGIC UPDATED: Use SURAH_NAMES Lookup ---
            surah_number = None
            
            # Clean the name for lookup (lowercase)
            clean_name = surah_name_raw.lower()
            
            # Direct lookup in your existing dictionary
            # Try 1: Exact match (e.g. "al-baqarah")
            if clean_name in SURAH_NAMES:
                surah_number = SURAH_NAMES[clean_name]
            
            # Try 2: Remove hyphens (e.g. "albaqarah") - looping if needed
            if not surah_number:
                clean_no_hyphen = clean_name.replace("-", "").replace("’", "")
                # Quick scan if keys in SURAH_NAMES match this
                for key, val in SURAH_NAMES.items():
                    if key.replace("-", "").replace("’", "") == clean_no_hyphen:
                        surah_number = val
                        break
            
            # Try 3: Partial match (e.g. "baqarah" inside "al-baqarah")
            if not surah_number:
                for key, val in SURAH_NAMES.items():
            ...
            # If still not found, check if the raw text actually contained a number like "Surah 18"
            if not surah_number:
                number_match = re.search(r'(?:Surah|surah)\s+(\d+)', response_text)
                if number_match:
                    surah_number = int(number_match.group(1))

            # Default to 1 only as a last resort
            if not surah_number:
                logging.warning(f"[EXTRACT_AUDIO] Could not match surah name '{surah_name_raw}', defaulting to 1")
                surah_number = 1 

            logging.info(f"[EXTRACT_AUDIO] Extracted - Surah: {surah_number} ({surah_name_raw}), Ayah: {ayah_match.group(1) if ayah_match else 'Full'}")
            ...
            return {
                ...
                "ayah_number": int(ayah_match.group(1)) if ayah_match else None,
