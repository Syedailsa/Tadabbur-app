...
# @tool
# async def play_quran_audio(query: str) -> str:
#     """
#     Play Quran audio recitation for requested surah or ayah.

#     Use this tool when user wants to:
#     - Listen to a surah (e.g., "play surah fatiha", "listen to surah 18")
#     - Listen to specific ayah (e.g., "play ayatul kursi", "surah 2 ayah 255")
#     - Hear Quran recitation

#     Args:
#         query: User's request containing surah/ayah information

#     Returns:
#         Audio URL and details for the requested recitation

#     Examples:
#         - "I want to listen to surah Fatiha"
#         - "Play Ayatul Kursi"
#         - "Can I hear surah 18?"
#         - "Listen to surah Kahf ayah 10"
#     """
#     import logging
...
#     if not parsed:
#         logging.warning("[AUDIO_TOOL] Failed to parse query")
#         return "I couldn't understand which surah or ayah you want to listen to. Please specify like 'Surah Fatiha' or 'Surah 2 Ayah 255'."

#     # Get audio
#     try:
#         logging.info(f"[AUDIO_TOOL] Calling get_quran_audio with surah={parsed['surah']}, ayah={parsed.get('ayah')}")
#         result = asyncio.run(get_quran_audio(
#             surah=parsed["surah"],
#             ayah=parsed.get("ayah"),
#             reciter="abdulbasit"
#         ))
#         logging.info(f"[AUDIO_TOOL] API result: success={result.get('success')}, type={result.get('type')}")

#         if not result["success"]:
#             logging.error(f"[AUDIO_TOOL] API failed: {result.get('error')}")
#             return f"Sorry, I couldn't fetch the audio: {result.get('error', 'Unknown error')}"
...
# You can click the link above to listen to the recitation.
# """.strip()
#             logging.info(f"[AUDIO_TOOL] Returning single ayah response with URL: {result['ayah']['audio_url']}")
#             return response

#         else:  # Complete surah
#             audio_list = "\n".join([f"  - Ayah {a['number']}: {a['audio_url']}" for a in result['ayahs'][:5]])
#             response = f"""
# Here is the audio for **Surah {result['surah']['englishName']}** (Complete - {result['surah']['numberOfAyahs']} ayahs):

# 🎙️ **Reciter:** {result['reciter']['name']}

# 🎧 **First few audio links:**
# {audio_list}

# (Showing first 5 ayahs. Full surah contains {result['surah']['numberOfAyahs']} ayahs)
# """.strip()
#             logging.info(f"[AUDIO_TOOL] Returning surah response with {len(result['ayahs'])} ayah URLs")
#             return response

#     except Exception as e:
#         logging.exception(f"[AUDIO_TOOL] Exception: {str(e)}")
#         return f"An error occurred while fetching audio: {str(e)}"
...
#     """
#     print("\n" + "="*60)
#     print("calling play_quran_audio ")
#     print(f"📥 user:  {query}")
#     print("="*60 + "\n")
...
#     try:
#         # Parse request
#         parsed = asyncio.run(parse_quran_audio_request(query))
#         logging.info(f"[AUDIO_TOOL] Parsed result: {parsed}")
...
async def play_quran_audio(query: str) -> str:
    ...
    try:
        ...
        parsed = await parse_quran_audio_request(query)
        logging.info(f"[AUDIO_TOOL] Parsed result: {parsed}")
