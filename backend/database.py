...
async def create_tables():
    ...
    async with get_db_connection() as conn:
        ...
        # ✅ REMOVE EXISTING UNIQUE CONSTRAINT FROM firstname (if it exists)
        # This will handle tables that already have the constraint
        try:
            await conn.execute("""
                ALTER TABLE users DROP CONSTRAINT IF EXISTS users_firstname_key;
            """)
            print("✅ Removed UNIQUE constraint from firstname")
        except Exception as e:
