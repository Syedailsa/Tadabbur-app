...
const BottomOptions = () => {
  ...
  const [storyTellingActive, setStoryTellingActive] = useState<boolean | null>(
    false
  );
    ...
    >
      ...
      <motion.div className={`ml-auto flex gap-x-1`}>
        ...
        <motion.div
          ...
          onClick={() => {
            setStoryTellingActive((prev) => !prev);
            const flag = { agent: "story-telling" };
            wsRef.current.send(JSON.stringify(flag));
          }}
          whileHover={{
          ...
          whileTap={{ backgroundColor: "#0000003D" }}
          animate={{
            backgroundColor: storyTellingActive ? "#0000000D" : "#00000000",
          }}
