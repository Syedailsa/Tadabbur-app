...
export default function ChatPage() {
  ...
  //   wsRef.current.onmessage = (event) => {
  //     try {
  //       const data = JSON.parse(event.data); // ✅ parse JSON from backend
  //       console.log("📩 Data from websocket:", data);

  //       switch (data.type) {
  //         case "connection_ack":
  //           console.log("✅ Connection acknowledged:", data.content);
  //           break;

  //         case "assistance_response":
  //           setMessages((prev) => [
  //             ...prev,
  //             { role: "assistant", content: data.content ?? "No reply from server" },
  //           ]);
  //           setLoading(false);
